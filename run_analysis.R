...
#Read in the activites for the training set and the test set and merge them.
...
## Now read in the subject identifier data for the training and the test datasets and merge them, as done above for the activity data.
...
## Finally, create a tidy data set with the average of each variable for each activity and each subject. I have used the ddply function from 'plyr' package for convenience. However, the same task can alternatively be performed with the 'by' function, but that does not produce a data frame.
...
write.table(names(tidy_dataset_average),file = "colnames.out",quote = F, col.names = F)
